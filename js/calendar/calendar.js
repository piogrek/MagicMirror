...
calendar.updateData = function (callback) {
    ...
    $.ajax("calendar.php").then(function (data) {
...
calendar.updateCalendar = function (eventList) {
    ...
    $(eventList).each(function (a, event) {

        var row = $('<tr>').css('opacity', opacity);
        row.append($('<td>').html(event.summary).addClass('description'));
        row.append($('<td>').html(event.start).addClass('days dimmed'));
        table.append(row);

        opacity -= 1 / eventList.length / 2;
    });
    $(this.calendarLocation).html(table.outerHTML());
